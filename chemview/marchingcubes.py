...
def make_field():
    ...
    subdivision = 32
    x = np.linspace(0, 4, subdivision)
    y = np.linspace(0, 4, subdivision)
    z = np.linspace(0, 4, subdivision)
    ...
    f = np.zeros((x.shape[0], y.shape[0], y.shape[0]), 'float32')
    for r, c in zip(radius, centers):
        f += ((xv-c[0])**2 + (yv-c[1])**2 + (zv-c[2])**2) - r**2
...
def main():
    ...
    triangles = marching_cubes(field, 0.0)
    print "Marching cubes complete"
    #print triangles
